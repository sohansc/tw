...
terraform output -state=./terraform/terraform.tfstate aws_instances_appserver_ip | sed -e s/,//g > ./terraform/hosts_appserver_${ENV}_${APP}.txt
terraform output -state=./terraform/terraform.tfstate aws_instances_webserver_ip | sed -e s/,//g > ./terraform/hosts_webserver_${ENV}_${APP}.txt
export ANSIBLE_HOST_KEY_CHECKING=False

#!/bin/bash 
        COUNTER=1
	Conn_check=`ansible --private-key $ANSIBLE_SSH_PRIVATE_KEY_FILE -u ubuntu -i ./terraform/hosts_appserver_${ENV}_${APP}.txt -m ping all`
	while [[  $COUNTER -lt 20  && ! $Conn_check =~ "SUCCESS" ]]; do
	     ...
	     Conn_check=$(echo `ansible --private-key $ANSIBLE_SSH_PRIVATE_KEY_FILE -u ubuntu -i ./terraform/hosts_appserver_${ENV}_${APP}.txt -m ping all`)
	     sleep 10;		
         done
...
ansible-playbook --private-key $ANSIBLE_SSH_PRIVATE_KEY_FILE -u ubuntu -i ./terraform/hosts_appserver_${ENV}_${APP}.txt ./ansible/tomcat-playbook/main.yml
ansible-playbook --private-key $ANSIBLE_SSH_PRIVATE_KEY_FILE -u ubuntu -i ./terraform/hosts_webserver_${ENV}_${APP}.txt ./ansible/apache2/main.yml
